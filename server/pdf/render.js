...
async function renderCoverLatexPdf({ payload, coverImage }) {
  ...
  if (coverImage?.buffer) {
    ...
    // fallback: create a blank 1x1 PNG so cover compilation never fails due to missing image
    const blank = Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO7Wn9kAAAAASUVORK5CYII=', 'base64');
    fs.writeFileSync(coverImagePath + '.png', blank);
  }
