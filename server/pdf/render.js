...
export async function renderFinanceMemoPdf({ payload, coverImage, logo, footerLogo, propertyImages }) {
  ...
  try {
    ...
    const pdf = await page.pdf({
      ...
      headerTemplate: '<div style="font-size:9px;width:100%;padding:0 14mm;color:#6b7280;"></div>',
      footerTemplate: `
        <div style="width:100%;padding:0 14mm;color:#9ca3af;display:flex;align-items:center;justify-content:space-between;font-family:Raleway, Arial, sans-serif;">
          <div style="display:flex;align-items:center;gap:10px;">
            ${viewModel.assets.footerLogo ? `<img src='${viewModel.assets.footerLogo}' style='height:18px;max-width:220px;object-fit:contain;'/>` : ''}
            <div style="font-size:9px;font-style:italic;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:520px;">
              ${escapeHtml(payload?.footers?.confidentiality || '')}
            </div>
          </div>
          <div style="font-size:9px;min-width:80px;text-align:right;">
