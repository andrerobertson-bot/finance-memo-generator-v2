...
const PayloadSchema = z
  .object({
    ...
    cover: z
      .object({
        // Cover bottom 40% (all editable; defaults match reference aesthetic)
        mainTitle: z.string().optional().default('Global Capital Commercial'),
        subheadline1: z.string().optional().default('Confidential'),
        subheadline2: z.string().optional().default('Finance Memorandum'),
        headline: z.string().optional().default('Construction Finance'),
        projectName: z.string().optional().default(''),

        // Company details block (editable; previously hardcoded)
        website: z.string().optional().default('globalcapital.com.au'),
        legalLine: z.string().optional().default('Global Capital Corporation Pty Ltd | ABN 14 097 482 114 | Telephone 612 9222 9100 | info@globalcapital.com.au'),
        addressLine: z.string().optional().default('Level 43 Governor Phillip Tower, 1 Farrer Place Sydney, NSW Australia 2000 | PO Box R196 Royal Exchange NSW 1225'),

        // Cover image behavior
        imageFitMode: z.enum(['smart', 'contain', 'cover']).optional().default('smart'),

        // Optional overlay toggles (in case user uploads a plain photo)
        showOverlayLogo: Bool.optional().default(false),
        showOverlayProposal: Bool.optional().default(false),
        proposalText: z.string().optional().default('PROPOSAL'),

        // Footer is OFF on cover by default (toggleable)
        showFooterOnCover: Bool.optional().default(false),
      })
      .default({}),
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const footerLogo = files.footerLogo?.[0] || null;
      const propertyImages = files.propertyImages || [];

      const pdfBuffer = await renderFinanceMemoPdf({ payload, coverImage, logo, footerLogo, propertyImages });
